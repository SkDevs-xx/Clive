...
```bash
# Claude Code CLI をインストール（公式手順に従う）
npm install -g @anthropic-ai/claude-code

# 認証（ブラウザが開く。VPS の場合は表示される URL を手元の PC で開く）
claude auth login
```
...
sudo systemctl enable disclaude   # OS 起動時に自動スタート
sudo systemctl start disclaude    # 今すぐ起動
```
...
```bash
journalctl -u disclaude -f        # リアルタイムでログを表示
journalctl -u disclaude --since "1 hour ago"  # 直近1時間のログ
```
...
# → http://localhost:6080/vnc.html でアクセス
```
